...
with col2:
    ...
    <style>
    .stApp {
        background: linear-gradient(135deg, #01030a, #0a0f1a);
    }
    h1, h2, h3, .stMarkdown h1, .stMarkdown h2, .stMarkdown h3 {
        color: #D4AF37 !important;
    }
...
# ======== Layout con Iustitia a la izquierda y texto centrado ========
col1, col2 = st.columns([1, 3])

with col1:
    st.image(JUSTITIA_URL, width=400)

with col2:
    st.markdown("""
    <div style="text-align: center;">
        <h1 style="color:#DDAF37; font-family: Georgia, 'Times New Roman', serif;
        font-size: 46px; margin-bottom: 30px; text-shadow: 2px 2px 6px #8B8680;">
        Jarquin Legal Services</h1>
        <h3 style="color:#DDAF37; font-size: 30px; font-family: Georgia, 'Times New Roman', serif;
        text-shadow: 1px 1px 4px #8B8680;">&amp; AI Solutions</h3>
        <h2 style="color:#DDAF37; font-size: 30px; font-family: Georgia, 'Times New Roman', serif;">
        INTELIGENCIA ESTRATÉGICA</h2>
        <h3 style="color:#FFE066; font-family: Georgia, 'Times New Roman', serif;">
        Unidad de Inteligencia Jurídica Digital (UIJD)</h3>
        <p style="color:#ffffff; font-family: Georgia, 'Times New Roman', serif;">
        Barrido Forense en 8 Capas - ICIJ + Datos Abiertos Costa Rica</p>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    st.markdown("""
        <div style="text-align: center;">
            <h1 style="color:#D4AF37; font-family: Georgia, 'Times New Roman', serif;
                       font-size: 46px; margin-bottom: 30px; text-shadow: 2px 2px 6px #b8860b;">
                Jarquin Legal Services<br>
                <small style="color:#D4AF37; font-size: 30px; font-family: Georgia, 'Times New Roman', serif;
                              text-shadow: 1px 1px 4px #b8860b;">
                    &amp; AI Solutions
                </small>
            </h1>
            <h2 style="color:#D4AF37; font-size: 30px; font-family: Georgia, 'Times New Roman', serif;">
                INTELIGENCIA ESTRATÉGICA</h2>
            <h3 style="color:#FFE066; font-family: Georgia, 'Times New Roman', serif;">
                Unidad de Inteligencia Jurídica Digital (UIJD)</h3>
            <p style="color:#ffffff; font-family: Georgia, 'Times New Roman', serif;">
...
if st.button("INICIAR INVESTIGACION FORENSE", type="primary"):
    if objetivo and len(objetivo.strip()) >= 3:
        # Incrementar contador para claves únicas
        st.session_state.download_counter += 1
        unique_key = st.session_state.download_counter
        ...
        with tabs[len(resultados)+2]:
            ...
            for ent in entidades_pendientes[:10]:
                with st.expander(ent):
                    nombre_manual = st.text_input("Nombre exacto", key=f"nom_{ent}_{unique_key}")
                    estado_manual = st.selectbox("Estado", ["", "INSCRITA", "MOROSA", "AL DIA"], key=f"est_{ent}_{unique_key}")
                    rep = st.text_area("Representantes", key=f"rep_{ent}_{unique_key}")
                    obs = st.text_area("Observaciones", key=f"obs_{ent}_{unique_key}")
                    if st.button("Guardar", key=f"btn_{ent}_{unique_key}"):
        ...
        for capa, hallazgos in resultados.items():
            for h in hallazgos:
                filas_csv.append({"Capa": capa, "Título": h['titulo'], "Fuente": h['fuente'], "Extracto": h['dato']})
        if filas_csv:
            ...
            writer.writerow(["No se encontraron hallazgos en las capas consultadas."])
            csv_buffer = io.BytesIO(buffer.getvalue().encode('utf-8-sig'))
        col_csv, col_pdf = st.columns(2)
        col_csv.download_button(
            "📥 Exportar CSV",
            data=csv_buffer,
            file_name=f"hallazgos_{objetivo.replace(' ', '_')}.csv",
            mime="text/csv",
            key=f"csv_btn_{unique_key}"
        )
        # Generar PDF con manejo de errores
        with st.spinner("Generando PDF, por favor espera..."):
            try:
                pdf_bytes = generar_pdf_premium(objetivo, resultados, datos_rn if isinstance(datos_rn, list) else [])
                if pdf_bytes:
                    col_pdf.download_button(
                        "📄 Descargar Dictamen PDF",
                        data=pdf_bytes,
                        file_name=f"Dictamen_{objetivo.replace(' ', '_')}.pdf",
                        mime="application/pdf",
                        key=f"pdf_btn_{unique_key}"
                    )
                else:
                    st.error("La generación del PDF falló. Exporta el CSV y contacta al soporte.")
            except Exception as e:
                st.error(f"❌ No se pudo generar el PDF: {e}")
                st.info("Puedes exportar los hallazgos a CSV y contactar al soporte si el problema persiste.")
                with st.expander("Ver detalles técnicos (para el desarrollador)"):
                    import traceback
                    st.code(traceback.format_exc())
    else:
